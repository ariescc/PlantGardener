<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="药物剂型" align="center">
        <template slot-scope="{row}">
          <span>{{ row.dosageForm }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 500px%; margin-left:50px;">
        <el-form-item label="药品图片">
          <img :src="temp.picture" class="avatar">
        </el-form-item>
        ...
        <el-form-item label="药物剂型">
          <el-input v-model="temp.dosageForm" />
        </el-form-item>
        <el-form-item label="药品规格">
          <el-input v-model="temp.specification" />
        </el-form-item>
        <el-form-item label="功效主治">
          <el-input v-model="temp.efficacy" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="Please input" />
        </el-form-item>
        <el-form-item label="使用方法"><!-- 文本框 -->
          <el-input v-model="temp.usageMethod" :autosize="{ minRows: 2, maxRows: 6}" type="textarea" placeholder="Please input" />
        </el-form-item>
        <el-form-item label="不良反应">
          <el-input v-model="temp.adverseReaction" :autosize="{ minRows: 2, maxRows: 6}" type="textarea" placeholder="Please input" />
        </el-form-item>
      </el-form>
      ...
      this.$axios
        ...
        .then(resp => {
          if (resp.data.code === 200) {
            console.log(resp)
            this.list = []
            resp.data.result.items.forEach(drug => {
              this.list.push({
                id: drug.id,
                name: drug.name,
    ...
    createData() {
      this.$axios
        .post('/adddrug', {
          ...
          dosageform: this.temp.dosageForm,
          specification: this.temp.specification,
          efficacy: this.temp.efficacy,
          usagemethod: this.temp.usageMethod,
          adversereaction: this.temp.adverseReaction
        })
        .then(resp => {
          console.log(resp)
    ...
    handleDetail(row) {
      this.$axios.get('/druginfo/' + row.id).then(resp => {
        ...
        if (resp && resp.data.code === 200) {
          ...
          this.temp.picture = resp.data.result.picture
        }
      })
